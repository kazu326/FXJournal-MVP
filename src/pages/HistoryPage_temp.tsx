    ...
    return (
        <div className="px-4 pb-24 h-full">
            ...
            {selectedLog && (
                ...
                >
                    <div
                        className="w-full bg-white rounded-t-3xl max-h-[85vh] flex flex-col overflow-hidden"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* モーダルヘッダー（固定） */}
                        <div className="flex-none flex items-center justify-between px-5 pt-5 pb-3 bg-white border-b border-zinc-100 z-10">
                            <div className="flex-1 overflow-hidden pr-2">
                                <p className="text-xs text-zinc-400 mb-0.5 truncate">
                            ...
                            <button
                                onClick={() => setSelectedLog(null)}
                                className="flex-none p-2 space-x-0 rounded-full bg-zinc-100 text-zinc-500 hover:bg-zinc-200 transition-colors"
                            >
                                <X size={18} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        ...
                                        return (
                                            <div
                                                key={key}
                                                className={`flex items-center justify-between px-3 py-2 rounded-xl text-sm ${displayState === "skip"
                            ...
                            {/* 仮説＆感情 */}
                            <section className="bg-zinc-50 rounded-2xl p-4 grid grid-cols-3 gap-2">
                                <div>
                                    <p className="text-[10px] text-zinc-400 font-bold mb-0.5 truncate">成功確率</p>
                                    <p className="text-sm font-bold text-zinc-700">
                                        {labelProb(selectedLog.success_prob)}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-[10px] text-zinc-400 font-bold mb-0.5 truncate">期待値</p>
                                    <p className="text-sm font-bold text-zinc-700">
                                        {labelEV(selectedLog.expected_value)}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-[10px] text-zinc-400 font-bold mb-0.5 truncate">感情スコア</p>
                                    <p className="text-sm font-bold text-zinc-700">
                                        {getEstimatedEmotionScore(selectedLog.occurred_at) ?? "−"}
                                    </p>
                                </div>
                            </section>
                            ...
                            {selectedLog.completed_at ? (
                                <section>
                                    ...
                                    <div className="space-y-2">
                                        <div
                                            className={`flex items-center justify-between px-4 py-3 rounded-xl border ${selectedLog.post_gate_kept
                                                    ? "bg-blue-50/50 border-blue-100 text-blue-700"
                                                    : "bg-red-50 border-red-100 text-red-600"
                                                }`}
                                        >
                                            <span className="text-sm font-medium">Gateの遵守</span>
                                            <span className="font-bold">
                                                {selectedLog.post_gate_kept ? "遵守した" : "違反した"}
                                            </span>
                                        </div>
                                        <div
                                            className={`flex items-center justify-between px-4 py-3 rounded-xl border ${selectedLog.post_within_hypothesis
                                                    ? "bg-emerald-50/50 border-emerald-100 text-emerald-700"
                                                    : "bg-orange-50 border-orange-100 text-orange-600"
                                                }`}
                                        >
                                            <span className="text-sm font-medium">相場は想定内</span>
                                            <span className="font-bold">
                                                {selectedLog.post_within_hypothesis ? "想定内" : "想定外"}
                                            </span>
                                        </div>
                                    </div>
                                </section>
                            ) : (
                                <div className="p-4 rounded-2xl border border-dashed border-zinc-200 text-center text-sm text-zinc-400">
                                    <Activity size={24} className="mx-auto mb-2 opacity-20" />
                                    取引後の振り返りはまだ記録されていません
                                </div>
                            )}
